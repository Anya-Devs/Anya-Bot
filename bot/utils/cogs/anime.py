...
class Manga_Recommendation(commands.Cog):
    def __init__(self, bot):
        self.bot = bot
        self.API_URL = "https://api.jikan.moe/v4/random/manga"

    async def fetch_random_manga(self) -> dict | None:
        async with aiohttp.ClientSession() as s:
            async with s.get(self.API_URL) as r:
                return await r.json() if r.status == 200 else None
    ...
    async def update_manga_embed(self, m: discord.Message, d: dict):
        a = d["data"]
        g = ", ".join(g["name"] for g in a.get("genres", [])) or "N/A"
        s = a.get("score") or 0
        stars = "▰" * int(s) + "▱" * (10 - int(s))
        ...
        e.add_field(
            name=" ",
            value=f"**Chapters:** `{a.get('chapters','N/A')}`\n"
                  f"**Status:** `{a.get('status','N/A')}`\n"
                  f"**Genres:** `{g if len(g) > 2 else 'Not Rated'}`\n"
                  + f"```py\nScore: {s:.1f}\n{stars}```",
            inline=False
        ...
        e.set_image(url=self.get_image_url(a.get("images", {})))
        e.set_footer(text="Manga Recommendation")

        v = View()
        v.add_item(Button(label="View Reviews", style=discord.ButtonStyle.link,
                          url=f"https://myanimelist.net/manga/{a.get('mal_id')}/reviews",
                          disabled=not a.get("mal_id")))
        b = Button(label="Get Another Random Manga", style=discord.ButtonStyle.primary, custom_id="random_manga_button")
        b.callback = self.random_manga_callback
        v.add_item(b)
        await m.edit(embed=e, view=v)

    async def random_manga_callback(self, i: discord.Interaction):
        d = await self.fetch_random_manga()
        await self.update_manga_embed(i.message, d)
        await i.response.defer()
