...
class PoketwoSpawnDetector(commands.Cog):
    ...
    async def output_prediction(self, dest, image_url):
        name, conf = self.predictor.predict(image_url)
        await dest.send(f"{self.format_name(name)}: {conf}")
        ...
        try:
            ...
            for e in m.embeds:
                if self.target_title in (e.title or "") and e.image:
                    return await self.output_prediction(m.channel, e.image.url)
        except Exception as e:
            await m.channel.send(f"❌ Error: {type(e).__name__}: {e}")
    ...
    async def predict_spawn(self, ctx, image_url=None):
        ...
        try:
            await self.output_prediction(ctx, image_url)
        except Exception as e:
