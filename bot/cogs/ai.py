...
class Ai(commands.Cog):
    def __init__(self, bot):
        ...
        if not self.OPENAI_KEY: raise ValueError("OPENAI_KEY missing in env.")
    ...
    async def imagine(self, ctx: commands.Context, *, prompt: str):
        if ctx.author.id in self.queue:
            return await ctx.send(Ai_Text.GENERATING_IMAGE.format(mention=ctx.author.mention))
        self.queue.append(ctx.author.id)
        try:
            async with ctx.typing():
                output_path = await self.image_gen.generate_image_sync(prompt)
                if not output_path: return await ctx.reply(Ai_Text.ERROR_GENERATING_PROMPT)
                final_path = self.image_gen.output_dir / "1.png"
                if final_path.exists(): final_path.unlink()
                output_path.rename(final_path)
                embed = discord.Embed(description=f"**Prompt:** ```{prompt}```", color=primary_color(), timestamp=datetime.now(), url="https://rajtech.me")
                embed.set_image(url="attachment://1.png")
                embed.set_footer(icon_url=ctx.author.avatar, text=f"Requested by {ctx.author}")
                await ctx.reply(embed=embed, file=discord.File(str(final_path), filename="1.png"))
    ...
    async def vision_command(self, ctx, image_url: str = None):
        prompt, message = Ai_Text.GENERATE_VISION_PROMPT, await ctx.reply(Ai_Text.THINKING_MESSAGE, mention_author=False)
        try:
            if not image_url:
                ref = ctx.message
                if ref.attachments: image_url = ref.attachments[0].url
                elif ref.reference:
                    ref_msg = await ctx.channel.fetch_message(ref.reference.message_id)
                    image_url = next((a.url for a in ref_msg.attachments), None) or \
                                next((e.thumbnail.url if e.thumbnail else e.image.url for e in ref_msg.embeds if e), None)
                elif ref.embeds:
                    embed = ref.embeds[0]
