...
class PoketwoSpawnDetector(commands.Cog):
        ...
        try:
            ...
            async with self._get_channel_lock(cid_str):
                ...
                if now - window_start > self.SPAM_WINDOW_SECONDS:
                    count = 1
                    window_start = now
                    # Do not reset ignored here; it persists until stats expire
                else:
                    count += 1
            ...
            for embed in message.embeds:
                ...
                if title and "pokémon has appeared!" in title.lower() and embed.image:
                    ...
                    if img_url:
                        try:
                            await self.queue.put((message, img_url), timeout=0.05)
    ...
    async def _pressure_loop(self, ctx: commands.Context, period: int) -> None:
        ...
        while self.testing:
            ...
            try:
                await self.queue.put((message, url), timeout=0.05)
            except asyncio.QueueFull:
                self.dropped_count += 1
