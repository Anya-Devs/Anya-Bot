...
class Config:
    ...
    def parse_shard_ids():
        ...
        for part in raw.split(","):
            ...
            if "-" in part: a,b=map(int,part.split("-",1)); ids.extend(range(a,b+1))
...
class ClusteredBot(commands.AutoShardedBot):
    def __init__(self):
        ...
        intents.presences=Config.USE_PRESENCE
        member_cache_flags=discord.MemberCacheFlags.none()
        allowed_mentions=discord.AllowedMentions(everyone=False,roles=False,users=True,replied_user=False)

        all_shards = Config.shard_count()
        cluster_shards = []
        if all_shards:
            per_cluster = all_shards // Config.CLUSTER_COUNT
            start = Config.CLUSTER_ID * per_cluster
            end = start + per_cluster
            cluster_shards = list(range(start,end))
        else:       
            cluster_shards = Config.parse_shard_ids()
            if cluster_shards is None:
              cluster_shards = [0]  #
        ...
        super().__init__(
            ...
            intents=intents,
            help_command=None,
            shard_ids=cluster_shards,
            shard_count=len(cluster_shards),
    ...
    async def setup_hook(self):
        ...
        try: self.http_session=aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=15))
        except Exception as e: logger.warning(f"aiohttp init failed: {e}")
    ...
    async def on_ready(self):
        ...
        banner+=f"🌐 Cluster {Config.CLUSTER_ID+1}/{Config.CLUSTER_COUNT} | Connected: {len(g)} servers | Users ~{sum(m.member_count or 0 for m in g)}".center(term)
        print(banner)
    ...
    async def _import_cogs(self):
        ...
        for dir_name in self.cog_dirs:
            ...
            except ModuleNotFoundError as e: branch.add(f"[red]Could not import {dir_name}: {e}[/red]"); continue
