...
class Dataset:
    def __init__(self, db_file='Data/pokemon/pokemon_images.db', processed_file='Data/pokemon/processed_pokemon_images.db', batch_size=5, image_folder='Data/pokemon/pokemon_images'):
        ...
        index_params = dict(algorithm=6, table_number=6, key_size=12, multi_probe_level=10, tree=6)
        ...
        for i in range(0, total_images, self.batch_size):
            ...
            for filename, result in zip(batch_paths, results):
                if result:
                    ...
                    # Serialize keypoints
                    keypoints_serialized = pickle.dumps(self.serialize_keypoints(keypoints))
                    descriptors_serialized = pickle.dumps(descriptors)
                    flipped_keypoints_serialized = pickle.dumps(self.serialize_keypoints(flipped_keypoints))
                    flipped_descriptors_serialized = pickle.dumps(flipped_descriptors)
                    
                    cursor.execute('''
                        INSERT OR REPLACE INTO images (filename, flipped_filename, keypoints, descriptors, flipped_keypoints, flipped_descriptors)
                        VALUES (?, ?, ?, ?, ?, ?)
                    ''', (os.path.basename(filename), flipped_filename, keypoints_serialized, descriptors_serialized, flipped_keypoints_serialized, flipped_descriptors_serialized))
            
            conn.commit()
            print(f"Processed batch {i//self.batch_size + 1} of {total_images//self.batch_size + 1}")
    ...
    def load_from_existing_db_in_batches(self):
        ...
        try:
            ...
            processed_entries = set(row[0] for row in processed_cursor.fetchall())
        ...
        for i in range(0, total_records, self.batch_size):
            ...
            for item in items:
                ...
                if filename not in processed_entries:
                    ...
                    # Mark as processed
                    processed_cursor.execute('INSERT OR IGNORE INTO processed (filename) VALUES (?)', (filename,))
            
            processed_conn.commit()
            print(f"Loaded batch {i//self.batch_size + 1} of {total_records//self.batch_size + 1}")
    ...
    def _get_cache_items(self):
        if not self.cache:
            conn = self._db_connect(self.db_file)
            cursor = conn.cursor()
            cursor.execute('SELECT filename, keypoints, descriptors, flipped_keypoints, flipped_descriptors FROM images')
            items = cursor.fetchall()
            conn.close()
            self.cache = {item[0]: (pickle.loads(item[1]), pickle.loads(item[2]), pickle.loads(item[3]), pickle.loads(item[4])) for item in items}
