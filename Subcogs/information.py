...
class SelectMenu(Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        try:
            ...
            page_num = 0
            await self.cog.navigate_role_members_embed(interaction, interaction.message, role_id, page_num)
...
class ButtonNavigationView(View):
    ...
    def update_buttons(self):
        self.clear_items()
        self.add_item(Button(label="◀️", style=discord.ButtonStyle.primary, custom_id="prev", disabled=self.current_page == 0))
        self.add_item(Button(label="▶️", style=discord.ButtonStyle.primary, custom_id="next", disabled=self.current_page == len(self.embeds) - 1))

    @discord.ui.button(label="◀️", style=discord.ButtonStyle.primary, custom_id="prev")
    async def previous_button_callback(self, interaction: discord.Interaction, button: Button):
    ...
    async def next_button_callback(self, interaction: discord.Interaction, button: Button):
...
class Guide(commands.Cog):
    ...
    async def navigate_role_members_embed(self, interaction, message, role_id, page_num):
     try:
        wait_message = None  # Initialize wait_message variable
        print(f"Navigation started for role ID: {role_id}, page number: {page_num}")
        ...
        if role:
            ...
            if embeds:
                ...
                if wait_message:  # Check if wait_message is not None
                    await wait_message.edit(content=f"`Completed`\n```Total embeds to navigate: {len(embeds)}```")
                ...
                if len(embeds) > 1:
                    ...
                    await message.edit(content="page updated", embed=embeds[0], view=view)
        ...
        await interaction.response.send_message(f"Error navigating through role members: {e}")
    
    async def navigate_embed(self, ctx, message, embeds):
     guild = ctx.guild
     roles = sorted(guild.roles, key=lambda role: (len(role.members), role.color.value), reverse=True)
     most_common_roles = [(role, len(role.members)) for role in roles if len(role.members) >= 5 and role.name]
     select_roles = most_common_roles[:25]  # Limit to 25 roles

     select_view = SelectView(roles=[role for role, _ in select_roles], cog=self)
     ...
     if len(embeds) > 1:
        ...
        for item in select_view.children:
            view.add_item(item)  # Add each item from SelectView to the main view
        ...
        await message.edit(content="Page updated", embed=embeds[self.current_page], view=view) 
    ...
    async def generate_role_members_embed(self, guild, role):
     try:
      ...
      for i in range(0, total_members, members_per_page):
        print(f"Generating embeds for page {i // members_per_page + 1}/{total_pages}")
        ...
        temp_concatenated_file = tempfile.NamedTemporaryFile(suffix=".png", delete=False)
        concatenated_image.save(temp_concatenated_file.name, format='PNG')
        ...
        with open(temp_concatenated_file.name, 'rb') as image_file:
            ...
            message = await log_channel.send(file=file)  # Send the file to the bot's DM
        ...
        for member in members_with_role[i:i + members_per_page]:
            ...
            joined_timestamp = self.timestamp_gen(member.joined_at.timestamp())
            other_roles = '\n'.join([f"➱ {r.mention}" for r in member.roles if r != role and r.name != "@everyone"])

            # Fetch invite information
            inviter_mention = await self.get_inviter_mention(member)  # Fetch invite information
    ...
    async def list_members(self, ctx, *, filter_option: str = None):
     try:
        ...
        if filter_option and filter_option.startswith(('-filter', '-fl', '-f', '-filter', '-fl', '-f')):
        ...
        if filter_option == FilterOption.NEWEST_TO_OLDEST.value:
            all_members = sorted(all_members, key=lambda member: member.joined_at, reverse=True)
            embed_title = "Server Member List (Newest to Oldest)"
        elif filter_option == FilterOption.OLDEST_TO_NEWEST.value:
            all_members = sorted(all_members, key=lambda member: member.joined_at)
            embed_title = "Server Member List (Oldest to Newest)"
        elif filter_option == FilterOption.MOST_ACTIVE_TO_LEAST_ACTIVE.value:
            all_members = sorted(all_members, key=lambda member: member.activity if isinstance(member.activity, int) else 0, reverse=True)
            ...
            all_members = sorted(all_members, key=lambda member: member.activity or 0)
            ...
            all_members = sorted(all_members, key=lambda member: len(member.roles), reverse=True)
            embed_title = "Server Member List (Most Roles to Least Roles)"
        elif filter_option == FilterOption.LEAST_ROLES_TO_MOST_ROLES.value:
            all_members = sorted(all_members, key=lambda member: len(member.roles))
        ...
        for member in all_members:
            if member.avatar and member.avatar.with_size(128):
                ...
                print(f"Skipping {member.display_name} due to invalid avatar")

        # Split valid members into chunks of 3 for embedding
        chunked_members = [valid_members[i:i + 3] for i in range(0, len(valid_members), 3)]
        ...
        for chunk in chunked_members:
            ...
            for member in chunk:
                ...
                temp_file = BytesIO()
                temp_file.write(data)
                temp_file.seek(0)  # Reset the position of the file cursor to the beginning of the file
                img = Image.open(temp_file)
                row_images.append(img)

                # Add member information to the embed
                embed = self.add_member_info_to_embed(embed, member, emoji, joined_timestamp, filter_option)
        ...
        error_message = f"Error listing members: {str(e)}\n```{traceback.format_exc()}```"
    ...
    async def create_member_embeds(self, chunked_members, ctx, embed_title, filter_option):
     embeds = []
     for chunk in chunked_members:
        embed = discord.Embed(title=embed_title)
        row_images = []
        ...
        for member in chunk:
            if not hasattr(member.avatar, 'with_size'):  # Check for NoneType attribute
                continue
            ...
            # Add member information to the embed
            embed = await self.add_member_info_to_embed(embed, member, emoji, joined_timestamp, filter_option)
    ...
    def concatenate_images(self, row_images):
     ...
     return temp_file
    def add_member_info_to_embed(self, embed, member, emoji, joined_timestamp, filter_option):
    ...
    async def on_message(self, message):
        ...
        if not message.author.bot:
            ...
            if author_id_str in self.data:
                ...
                await message.channel.send(f"Welcome back {message.author.mention}, I removed your AFK!", delete_after=35)
            ...
            for user_id, afk_reason in dict(self.data).items():
                if f"<@{user_id}>" in message.content:
                    await message.channel.send(f"<@{user_id}> is currently AFK, reason: {afk_reason}", delete_after=35)
   
    @commands.command(name="lock", help="Lock channels (Role required: Lock)",hidden=True)
    ...
    async def lock(self, ctx, channel: discord.TextChannel = None):
        ...
        if incense in channel.name or ctx.author.id == 1030285330739363880 or ctx.author.guild_permissions.manage_channels:
            await channel.set_permissions(ctx.guild.default_role, overwrite=overwrite)
            await ctx.send(f'{ctx.author.mention} has locked {channel.mention}, {channel.mention} is now :lock: ')
            await channel.set_permissions(ctx.guild.default_role, overwrite=overwrite)
        else:
            await ctx.send(f"{ctx.author.name}, I am sorry but you don't have the permission to lock {channel.mention}. ||Missing Permission: [manage_channels]||. Try locking channels called incense for now.")
    
    @commands.command(name="unlock", help="Unlock channels (Role required: Lock)",hidden=True)
    @commands.has_role('Lock')
    ...
    async def lock_error(self, ctx, error):
        if isinstance(error, (commands.MissingRole, commands.MissingAnyRole)):
            text = "Sorry {}, you don't have the correct role to lock channels. || Role required = Role Name: 'Lock' || ".format(ctx.message.author)
