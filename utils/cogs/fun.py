...
class Fun_Commands:
    ...
    async def action_command(self, ctx, user: Union[discord.Member, Literal["everyone"]], additional_text=""):
     ...
     if isinstance(user, str) and user.lower() == 'everyone': action_target, action_phrase = "the whole server", self.phrases["everyone"].get(action, f"{action}s")
     elif isinstance(user, discord.Member) and user == ctx.author: action_target, action_phrase = "themselves", self.phrases["self"].get(action, f"{action}s")
     else: action_target, action_phrase = user.display_name, self.phrases["other"].get(action, f"{action}s")
     ...
     elif user != ctx.author and not ctx.author.bot and not isinstance(user, str) and user != "everyone" and not user.bot: await self.mongo.update_one({"server_id": sid}, {"$inc": {f"members.{action}.outake.{aid}": 1}}, upsert=True)
