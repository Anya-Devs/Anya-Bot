...
class Fun_Commands:
    ...
    async def action_command(self, ctx, user: Union[discord.Member, Literal["everyone"]], additional_text=""):
     ...
     action_phrases = {
        "pat": "gives a gentle pat to", "cuddle": "cuddles", "kiss": "gives a kiss to", "hug": "hugs",
        "bite": "bites", "lick": "licks", "slap": "slaps", "cry": "is crying", "slowclap": "gives a slow clap",
        "wave": "waves at", "smug": "gives a smug look to"
     }
     action_phrase = action_phrases.get(action, f"{action}s")
     msg = f"{ctx.author.display_name} {action_phrase} the whole server {emote} {additional_text}" if isinstance(user, str) and user.lower() == "everyone" else f"{ctx.author.display_name} {action_phrase} {user.display_name} {emote} {additional_text}"
     embed = discord.Embed(title=msg.strip(), color=primary_color()).set_image(url=gif)
     sid, aid = str(ctx.guild.id), str(ctx.author.id)
     if not isinstance(user, str) and user != ctx.author:
        await self.mongo.update_one({"server_id": sid}, {"$inc": {f"members.intake.{str(user.id)}": 1}}, upsert=True)
     ...
     sent, received = doc.get("members", {}).get("outake", {}).get(aid, 0), doc.get("members", {}).get("intake", {}).get(aid, 0)
