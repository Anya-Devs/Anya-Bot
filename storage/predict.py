...
class PokemonPredictor:
    def __init__(
        ...
        self.cache = {}  # Only store descriptors
    ...
    def load_from_npy(self, dataset_file):
        ...
        self.cache = data  # This will only be descriptors now
        print(
            f"Loaded dataset from {dataset_file}. Total images: {len(self.sub_image_filenames)}"
        )
    ...
    def process_image(self, path, filename):
        ...
        if img is not None:
            ...
            if descriptors is not None:
                self.cache[filename] = descriptors.astype(
                    np.uint8
                )  # Store descriptors as uint8
                self.distance_cache[filename] = (
    ...
    def cache_flipped_image(self, img, filename):
        ...
        if descriptors is not None:
            flipped_filename = filename.replace(
                ".png", "_flipped.png"
            )  # Keep flipped filename for cache
            self.cache[flipped_filename] = descriptors.astype(
                np.uint8
            )  # Store descriptors as uint8
            self.distance_cache[flipped_filename] = (
                descriptors  # Store distances for future use
            )
    ...
    def cross_match(self, img, desB, k=2):
        ...
        # Ensure desB is a numpy array
        desB = np.asarray(desB, dtype=np.uint8)

        # Compute matches using descriptors
        futures = [
            self.executor.submit(
                self.flann.knnMatch, desB, desA, k
            )  # Use descriptors directly
            for desA in self.cache.values()  # Get the descriptors
        ]
                    ...
                    async with session.get(image_url) as response:
                        if response.status == 200:
                            ...
                            if hunters:
                                ...
                                hunter_mentions = " ".join(
                                    [f"<@{hunter_id}>" for hunter_id in hunters]
                                )  # Updated format for mentions
    ...
    async def hunt(self, ctx, action="list", *pokemon_names):
        ...
        if action == "list":
            ...
            if user_pokemon:
                ...
                await ctx.reply(
                    "### You don't have any Pokémon in your hunting list yet!\n- `Try doing ...hunt add <pokemon_name>`",
                    mention_author=False,
                )
            ...
            if len(user_pokemon) + len(pokemon_names) > 10:
                await ctx.reply(
                    "You already have 10 Pokémon in your hunt list. Please **remove** one to add new ones.",
                    mention_author=False,
                )
                return
            ...
            await ctx.reply(
                "Invalid action! Use `list`, `add`, or `remove`.", mention_author=False
            )
    ...
    async def send_pokemon_info(self, ctx, data, type, color):
        ...
        def find_pokemon_weaknesses(pokemon_info, type_chart):
            ...
            for pokemon_type in types:
                weaknesses.update(
                    type_chart.get(pokemon_type, {}).get("double_damage_from", [])
                )
                strengths.update(
                    type_chart.get(pokemon_type, {}).get("double_damage_to", [])
                )
     ...
     """
        ...
        wes = result.format("", "", strengths_formatted, "", weaknesses_formatted, "")
            ...
            if pokemon_data:
                ...
                if names_field:
                    ...
                    sorted_names_by_length = dict(
                        sorted(alt_names_info.items(), key=lambda item: len(item[1]))
                    )

                    if len(sorted_names_by_length) != len(alt_names_info):
                        sorted_names_by_name = dict(
                            sorted(alt_names_info.items(), key=lambda item: item[1])
                        )
