...
class PokemonPredictor:
    def __init__(self, dataset_folder="Data/pokemon/pokemon_images", csv_file="Data/pokemon/dataset.csv", approx_file="Data/pokemon/pokemon_aprox.csv"):
    ...
    def _find_image_inside_image(self, img, template):
        ...
        if loc[0].size > 0:
            ...
            for pt in zip(*loc[::-1]):  # Switch columns and rows
                ...
                return [pt[0], pt[1], bottom_right[0], bottom_right[1]], os.path.basename(template)
    ...
    def predict_pokemon(self, img):
        ...
        if results:
            ...
            if target_img is not None:
                ...
                detection_box, filename = self._find_image_inside_image(img, target_img)
                ...
                if detection_box is not None:
                    ...
                    predicted_pokemon = filename.split("_flipped")[0].replace(".png", "")
                    return f"{predicted_pokemon.title()}: 100%", round(time.time() - start_time, 2)
                else:
                    print(f"Warning: No detection found for {pokemon_img_path}")
                    predicted_pokemon = pokemon_img_path.split("_flipped")[0].replace(".png", "").replace(f"{self.dataset_folder}","")
                    return f"{predicted_pokemon.title()}: 100%", round(time.time() - start_time, 2)
