...
logging.basicConfig(
    level=logging.ERROR,  # Set the logging level (can be DEBUG, INFO, WARNING, ERROR, CRITICAL)
...
class Anime(commands.Cog):
    ...
    async def character_search(self, ctx, *, query=None):
        if query is None:
            ...
            try:
                ...
                timeout_embed = discord.Embed(description="Time's Up! You didn't provide a character name for me to look up.", color=discord.Color.red())
                await message.edit(embed=timeout_embed)
                return
        ...
        try:
            ...
            view = self.CharacterView(character_data, current_index, current_page)
            embed = await view.update_embed()
            message = await ctx.reply(embed=embed, view=view, mention_author=False)
    ...
    async def manga_search(self, ctx, *, query=None):
        if query is None:
            embed = discord.Embed(description=f"{ctx.author.display_name.title()}, can you try entering the `name` of the manga you're looking for?")
            message = await ctx.reply(embed=embed, mention_author=False)
            ...
            try:
                ...
                timeout_embed = discord.Embed(description="Time's Up! You didn't provide a manga title for me to look up.", color=discord.Color.red())
                await message.edit(embed=timeout_embed)
                return
    ...
    class AnimeView(discord.ui.View):
        ...
        async def update_embed(self):
            ...
            embed.description = anime.get('synopsis', "> <:anya_angy:1268976144548630608> Synopsis not available")
    ...
    class CharacterButton(discord.ui.Button):
        def __init__(self, label, style, custom_id, character_data, current_index, current_page, max_pages):
            super().__init__(label=label, style=style, custom_id=custom_id)
        ...
        async def callback(self, interaction: discord.Interaction):
            ...
            view = Anime.CharacterView(self.character_data, self.current_index, self.current_page)
            embed = await view.update_embed()
            await interaction.response.edit_message(embed=embed, view=view)
    ...
    class CharacterView(discord.ui.View):
        ...
        def update_buttons(self):
            self.clear_items()
            if self.current_page > 0:
                self.add_item(Anime.CharacterButton("Previous", discord.ButtonStyle.primary, "previous", self.character_data, self.current_index, self.current_page, self.max_pages))
            if self.current_page < self.max_pages:
                self.add_item(Anime.CharacterButton("Next", discord.ButtonStyle.primary, "next", self.character_data, self.current_index, self.current_page, self.max_pages))
    ...
    class MangaView(discord.ui.View):
     ...
     def update_buttons(self):
        ...
        if self.current_page > 0:
            self.add_item(Anime.MangaButton("Previous", discord.ButtonStyle.primary, "previous"))
        if self.current_page < self.max_pages:
            self.add_item(Anime.MangaButton("Next", discord.ButtonStyle.primary, "next"))
     ...
     async def update_embed(self):
        ...
        embed.description = manga.get('synopsis', "> <:anya_angy:1268976144548630608> Synopsis not available")
        embed.set_footer(text=f"Page {self.current_page + 1}/{self.max_pages + 1}")
