...
class DiscordMessageConfig:
    ...
    def __init__(
        ...
        # Discord username color (light blue)
...
class DiscordMessageImage:
    ...
    def create_image(self):
        ...
        temp_image = Image.new(
            "RGB", (self.config.canvas_width,
                    1000), self.config.background_color
        )
        ...
        total_height = (
            max(self.config.avatar_size[1],
                message_height) + self.config.padding * 2
        )
        ...
        if avatar:
            canvas.paste(avatar, (self.config.padding,
                         self.config.padding), avatar)

        # Draw the username in Discord-style blue and the timestamp to the right
        username_x = (
            self.config.padding +
            self.config.avatar_size[0] + self.config.padding
        )
...
class MessageImageCog(commands.Cog):
    ...
    async def test(self, ctx, *, text: str):
        ...
        config = DiscordMessageConfig(
            ...
            message=f"{text}",
            # Corrected the access to avatar URL
