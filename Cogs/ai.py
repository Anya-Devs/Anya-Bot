...
class Ai(commands.Cog):
    ...
    @commands.command(
        name="vision_model",
        aliases=["vm"],
        description="Change the vision model identifier or show the current model",
        hidden=True,
    )
    async def set_vision_model(self, ctx: commands.Context, model_id: str = None):
        ...
        if model_id:
            ...
            await ctx.send(
                f"Current vision model: {current_model}\n\n```{model_list_message}```"
            )
...
class VisionModelOptions:
    ...
    def get_model_embed(self) -> discord.Embed:
        ...
        embed = discord.Embed(
            ...
            description="Choose a vision model by replying with the number corresponding to the model.",
            color=0x00FF00,
        )
        for i, model in enumerate(self.models):
            embed.add_field(name=f"{i + 1}. {model}",
                            value="\u200b", inline=False)
        return embed
