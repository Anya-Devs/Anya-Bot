...
def timestamp_gen(timestamp: int) -> str:
    dt = datetime.datetime.utcfromtimestamp(timestamp).replace(
...
class Anti_Thief(commands.Cog):
    def __init__(self, bot):
        ...
        self.bot_id = [874910942490677270,
                       1234247716243112100, 854233015475109888]
        self.shiny_hunters = []
    ...
    async def process_pings(self, guild, message_content):
        ...
        if self.shiny_ping_phrase in message_content:
            mention_start_index = message_content.find(self.shiny_ping_phrase) + len(
                self.shiny_ping_phrase
            )
    ...
    async def set_ping_phrase(self, ctx, *, new_phrase: str):
        ...
        await ctx.reply(
            f"Shiny hunt ping phrase updated to: {new_phrase}", mention_author=False
        )

    @commands.Cog.listener()
    async def on_message(self, message):
        if message.author.id in self.bot_id and message.guild:
            self.shiny_hunters = await self.process_pings(
...
class EventGate(commands.Cog):
    ...
    async def send_shiny_hunt_embed(
        self, channel, shiny_hunters, reference_message=None
    ):
        if channel.id in self.active_events:
            logger.info(
                f"Shiny hunt already active in channel {channel.id}. Skipping new event."
            )
            return
    ...
    async def start_countdown(self, message, wait_until, reference_message):
        try:
            await asyncio.wait_for(
                self.wait_for_congratulations(
                    message, wait_until, reference_message),
                timeout=self.wait_time,
            )
        except asyncio.TimeoutError:
            logger.warning(
                f"Timeout reached in channel {message.channel.id}. Allowing all to catch."
            )
    ...
    async def wait_for_congratulations(self, message, wait_until, reference_message):
        def check(m):
            ...
            if m.author.id == self.detect_bot_id and m.id not in self.handled_congrats:
                match = re.match(
                    r"Congratulations <@(\d+)>! You caught a Level \d+ .+", m.content
                )
                if match:
                    return True
            return False
        ...
        try:
            ...
            await self.process_congratulations(
                congrats_message, message, reference_message
            )
        except asyncio.TimeoutError:
            logger.warning(
                f"Timeout: No congratulatory message detected in channel {message.channel.id}."
            )
            await self.allow_all_to_catch(message)
        except Exception as e:
    ...
    async def process_congratulations(
        self, congrats_message, original_message, reference_message
    ):
        try:
            self.shiny_ping_phrase = load_ping_phrase()
            ...
            logger.info(
                f"Reference message content: {reference_message.content}")

            mentioned_user_id = re.search(r"<@(\d+)>", reference_message.content).group(
                1
            )
            who_caught_pokemon_user_id = re.search(
                r"<@(\d+)>", congrats_message.content
            ).group(1)

            if who_caught_pokemon_user_id not in congrats_message.content:
                logger.warning(
                    f"Something weird happened in {catch_channel.channel.name}."
                )
            ...
            if self.shiny_ping_phrase in reference_message.content:
                quest_pings_content = reference_message.content.split(
                    self.shiny_ping_phrase
                )[1].strip()
                quest_user_ids = re.findall(r"<@(\d+)>", quest_pings_content)
                logger.info(
                    f"Extracted Shiny Pings user IDs: {quest_user_ids}")

            if who_caught_pokemon_user_id not in quest_user_ids:
                logger.warning(
                    f"User {who_caught_pokemon_user_id} is not in the quest pings list."
                )
                is_shiny_hunter = await self.bot.get_cog("Anti_Thief").is_shiny_hunter(
                    int(who_caught_pokemon_user_id)
                )

                if not is_shiny_hunter:
                    logger.info(
                        f"User {who_caught_pokemon_user_id} is not a shiny hunter. Logging incident."
                    )
                    non_hunter = await self.bot.fetch_user(who_caught_pokemon_user_id)

                    p_match = re.search(r"Level \d+ (\w+)",
                                        congrats_message.content)
                    pokemon_name = p_match.group(
                        1) if p_match else "Unknown Pokémon"
                    ...
                    logger.info(
                        f"Non-hunter {non_hunter.mention} detected and timed out."
                    )

                    logger_channel = self.bot.get_channel(
                        self.logger_channel_id)
                    log_embed = Embed(
                        title="Shiny Theft",
                        description=(
                    ...
                    log_embed.set_footer(
                        icon_url=self.bot.user.avatar, text="Anya Logger"
                    )
                    await logger_channel.send(embed=log_embed)
                    return

            logger.info(
                f"User {who_caught_pokemon_user_id} is valid. Proceeding with congratulations."
            )
            shiny_hunters = await self.bot.get_cog("Anti_Thief").process_pings(
                reference_message.guild.id, reference_message.content
            )
            shiny_hunter = next(
                (
                    hunter
                    for hunter in shiny_hunters
                    if str(hunter.id) == mentioned_user_id
                ),
                None,
            )
            ...
            if shiny_hunter:
                ...
                logger.warning(
                    f"No shiny hunter found for user {mentioned_user_id}.")
                await catch_channel.send(
                    "⚠️ No shiny hunter detected for the quest. Please double-check."
                )
    ...
    async def timeout_user(self, user, message):
        ...
        timeout_end = datetime.datetime.utcnow() + datetime.timedelta(
        ...
        if response.status_code == 204:
            ...
            logger.error(
                f"Failed to timeout user {user.mention}: {response.status_code}"
            )
