...
class Quest_Checker(commands.Cog):
                ...
                if quest["action"] == "send":
                    if int(quest["channel_id"]) == message.channel.id:
                        if quest["method"] == "message":
                            await self.handle_message_quest(
                                quest, message, user_id, guild_id
                            )
                        elif quest["method"] == "emoji":
                            await self.handle_emoji_quest(
                                quest, message, user_id, guild_id
                            )
    ...
    async def handle_message_quest(self, quest, message, user_id, guild_id):
        ...
        normalized_quest_content = re.sub(
        ...
        if similarity_ratio >= 88:
            ...
            if "{member}" not in quest_content or (
                ...
                await self.update_quest_progress(
                    guild_id, user_id, quest["quest_id"], quest["progress"]
                )
    ...
    async def handle_emoji_quest(self, quest, message, user_id, guild_id):
        ...
        if isinstance(quest_emoji, int):
            quest_emoji = str(discord.utils.get(
                self.bot.emojis, id=int(quest_emoji)))
    ...
    async def complete_quest(
        ...
        try:
            logger.debug(
                f"Completing quest: {quest_id} for user_id: {user_id} in guild_id: {guild_id}"
            )
            ...
            logger.debug(
                f"Quest {quest['quest_id']} deleted for user {user_id} in guild {guild_id}."
            )
    ...
    async def update_quest_progress(self, guild_id, user_id, quest_id, progress):
        try:
            logger.debug(
                f"Updating progress for quest_id: {quest_id} for user_id: {user_id} in guild_id: {guild_id} to progress: {progress}"
            )
            await self.quest_data.update_quest_progress(
                guild_id, user_id, quest_id, progress
            )
            logger.debug(
