...
class DatabaseManager:
    ...
    async def get_balance(self, user_id: str, guild_id: str):
        ...
        logger.debug(
            f"Fetching balance for user {user_id} in guild {guild_id}")
        try:
            ...
            logger.error(
                f"Error occurred while getting balance for {user_id} in guild {guild_id}: {e}"
            )
            return 0
    ...
    async def update_balance(self, user_id: str, guild_id: str, new_balance: float):
        ...
        logger.debug(
            f"Updating balance for user {user_id} in guild {guild_id} to {new_balance}"
        )
        try:
            server_collection = self.db["Servers"]
            ...
            if result.modified_count > 0:
                logger.info(
                    f"Updated balance for user {user_id} in guild {guild_id}. New balance: {new_balance}"
                )
            else:
                logger.error(
                    f"Failed to update balance for user {user_id} in guild {guild_id}."
                )
        except PyMongoError as e:
            logger.error(
                f"Error occurred while updating balance for {user_id} in guild {guild_id}: {e}"
            )

    async def add_stolen_balance(
        self, user_id: str, guild_id: str, stolen_points: float
    ):
        """Add stolen balance (siphoned points) to the author's balance."""
        logger.debug(
            f"Adding stolen balance of {stolen_points} to user {user_id} in guild {guild_id}"
        )
        try:
            ...
            if result.modified_count > 0:
                ...
                logger.info(
                    f"Successfully added stolen points ({stolen_points}) to user {user_id}'s balance. New balance: {new_balance}"
                )
            else:
                logger.error(
                    f"Failed to add stolen points for user {user_id} in guild {guild_id}."
                )
        except PyMongoError as e:
            logger.error(
                f"Error occurred while adding stolen points for {user_id} in guild {guild_id}: {e}"
            )
...
class AffectedUser:
    ...
    async def apply_tool(self, tool_name: str):
        ...
        logger.debug(
            f"Applying {tool_name} for user {self.user_id} in guild {self.guild_id}"
        )
    ...
    async def apply_motion_alarm(self, user_key: str, current_time: datetime):
        ...
        if duration > 0:
            ...
            self.redis_client.setex(
                f"{user_key}:motion_alarm", duration, expiration_time.timestamp()
            )
            logger.info(
                f"Motion Alarm applied to {self.user_id} for {duration} seconds."
            )
    ...
    async def apply_key_chain_sheep(self, user_key: str):
        ...
        logger.info(
            f"Key Chain Sheep effect logged for {self.user_id} in guild {self.guild_id}."
        )
...
class Server:
    ...
    def get_affected_user(self, user_id: str):
        ...
        if user_id not in self.affected_users:
            self.affected_users[user_id] = AffectedUser(
                self.guild_id, user_id, self.redis_client
            )
...
class ToolHandler:
    ...
    async def handle_chimera(self, user_id, channel):
        ...
        if effect:
            logger.info(
                f"Chimera tool applied successfully for {user_id} with effect: {effect}"
            )
            await channel.send(
                f"<@{user_id}> has been affected by Chimera! Effect: {effect}"
            )
        else:
    ...
    async def handle_spy_briefcase(self, author_id, user_id, channel):
        ...
        logger.debug(
            f"Handling Spy Briefcase for {author_id} in channel {channel.id}")
        guild_id = str(channel.guild.id)

        # Check if user has the tool in their inventory
        tool_inventory = await self.check_inventory(
        ...
        mentioned_user_id = str(
            channel.last_message.mentions[0].id
        )  # Taking the last mentioned user

        # Get balances
        author_balance = await self.db_manager.get_balance(
            author_id, guild_id
        )  # Balance of the author
        target_balance = await self.db_manager.get_balance(
            mentioned_user_id, guild_id
        )  # Balance of the mentioned user
        ...
        if siphoned_points > 0:
            ...
            new_author_balance = (
                author_balance + siphoned_points
            )  # Add siphoned points to the author's balance

            # Update target balance
            await self.db_manager.update_balance(
                mentioned_user_id, guild_id, new_target_balance
            )

            # Add siphoned points to author's balance
            await self.db_manager.add_stolen_balance(
                author_id, guild_id, siphoned_points
            )

            logger.info(
                f"Siphoned {siphoned_points} points from {mentioned_user_id} to {author_id}."
            )
            await channel.send(
    ...
    async def handle_key_chain_sheep(self, author_id, user_id, channel):
        ...
        logger.debug(
            f"Handling Key Chain Sheep for {user_id} in channel {channel.id}")
        ...
        if not tool_inventory:
            ...
            await channel.send(
                f"<@{author_id}>, you do not have the {tool_name} tool in your inventory."
            )
            return
        ...
        if not roles:
            ...
            await channel.send(
                "No roles found for this server. Please ensure roles are properly set."
            )
            return
        ...
        if random_role:
            ...
            if target:
                ...
                await channel.send(
                    f"<:keychain_sheep:1318415294758129692> Assigned the role **{random_role.name}** to {target.name}."
                )
                logger.info(
                    f"<:keychain_sheep:1318415294758129692> Assigned the role **{random_role.name}** to {target.name}."
                )
                ...
                logger.warning(
                    f"Target user {user_id} not found in the guild.")
                await channel.send(
                    f"Could not find the user <@{user_id}> in the guild."
                )
        else:
            logger.warning(
                f"The role {random_role_id} could not be found in the guild."
            )
            await channel.send(
                "The selected role could not be found. Please ensure the roles are set correctly."
            )
    ...
    async def check_inventory(self, user_id: str, guild_id: str, tool: str):
        ...
        logger.debug(
            f"Checking inventory for user {user_id} in guild {guild_id} for tool {tool}"
        )
        ...
        try:
            ...
            if not user_data:
                logger.warning(
                    f"No data found for user {user_id} in guild {guild_id}.")
                return False

            # Extract the inventory from the nested structure
            inventory = (
                user_data.get("members", {}).get(
                    str(user_id), {}).get("inventory", {})
            )
            logger.debug(f"Inventory: {inventory}")

            # Check for the tool in the nested 'tool' section of the inventory
            # Access the nested 'tool' dictionary
            tools = inventory.get("tool", {})
            tool_lower = tool.lower()
            matching_tool = next(
                (key for key in tools if key.lower() == tool_lower), None
            )

            if matching_tool:
                logger.debug(
                    f"Tool {tool} found in inventory for user {user_id}.")
                ...
                logger.info(
                    f"User has {quantity} of {tool} (Tool ID: {un_tool_id})")

                # Validate quantity
                if quantity > 0:
                    logger.info(
                        f"User has the required {tool} with sufficient quantity."
                    )

                    # Remove 1 from the inventory
                    success = await self.remove_one_from_inventory(
                        user_id, guild_id, tool
                    )
                    if success:
                        logger.info(
                            f"Successfully removed 1 {tool} from user {user_id}'s inventory."
                        )
                        return True
                    else:
                        logger.warning(
                            f"Failed to remove 1 {tool} from user {user_id}'s inventory."
                        )
                        return False
                else:
                    logger.warning(
                        f"Tool {tool} found but insufficient quantity ({quantity})."
                    )
                    return False
            else:
                logger.warning(
                    f"Tool {tool} not found in inventory for user {user_id}."
                )
                return False
        except Exception as e:
            logger.error(
                f"Error checking inventory for user {user_id} in guild {guild_id}: {e}"
            )
            return False
    ...
    async def remove_one_from_inventory(self, user_id: str, guild_id: str, tool: str):
        ...
        logger.debug(
            f"Removing 1 quantity of {tool} from user {user_id} in guild {guild_id}."
        )
        ...
        try:
            ...
            if current_quantity <= 0:
                logger.warning(
                    f"Insufficient quantity of {tool} for user {user_id}. Current quantity: {current_quantity}."
                )
                return False
            ...
            logger.info(
                f"Decremented {tool} quantity by 1 for user {user_id}. New quantity: {new_quantity}."
            )
            return True

        except Exception as e:
            logger.error(
                f"Error removing 1 quantity of {tool} for user {user_id} in guild {guild_id}: {e}"
            )
            return False
    ...
    async def apply_tool_effect(self, author_id, user_id, tool, channel):
        ...
        logger.debug(
            f"Applying tool effect: {tool} for user {user_id} in channel {channel.id}"
        )
        tool = tool.title()
        ...
        if tool == "Motion Alarm":
            ...
            await channel.send(
                f"Tool {tool} not recognized. Please check the tool name."
            )
...
class ToolBot(commands.Cog):
    ...
    async def on_message(self, message: discord.Message):
        ...
        try:
            ...
            mentioned_user = (
                message.mentions[0] if message.mentions else None
            )  # Get the mentioned user

            # Log detected tool ID and mentioned user ID
            if mentioned_user:
                print(
                    f"Tool_ID: {tool_id_str} detected in message {message.id}")
                ...
                user_id = str(
                    message.author.id
                )  # Fetching the inventory of the message author
                db = self.quest_data.mongoConnect[self.quest_data.DB_NAME]
                server_collection = db["Servers"]
                ...
                if inventory:
                    print(
                        f"Inventory for User {message.author.display_name} (ID: {message.author.id}):"
                    )
                    for tool, tool_data in inventory.items():
                        tool_name = (
                            tool.capitalize()
                        )  # Assuming tool name is the tool key (e.g., "hammer")
                        tool_id_in_inventory = (
                            await self.quest_data.get_existing_tool_id(
                        ...
                        print(
                            f" - Tool: {tool_name} (Tool_ID: {tool_id_in_inventory})")
                        ...
                        if str(tool_id_in_inventory) == tool_id_str:
                            print(
                                f"Match found: Tool ID {tool_id_str} matches {tool_name}"
                            )
                            # Apply the tool effect to the mentioned user
                            await self.tool_handler.apply_tool_effect(
                                message.author.id,
                                mentioned_user.id,
                                tool_name,
                                message.channel,
                        ...
                        print(
                            f"No match found for Tool_ID: {tool_id_str} in the inventory."
                        )
                else:
                    print(
                        f"User {message.author.display_name} (ID: {message.author.id}) has an empty inventory."
                    )
